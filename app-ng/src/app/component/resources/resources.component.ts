...
export class ResourcesComponent implements OnInit  {
  ...
  ngOnInit(): void {
    ...
    const notesData = localStorage.getItem('reseources');
    if (notesData) {
      this.resources = JSON.parse(notesData);
    } else {
  ...
  private refreshData(): void {
    ...
    this.refreshSubscription = interval(5000)
      .pipe(switchMap(() =>  this.resourcesService.getAllResourcesPages(this.currentPage, this.size)))
      .subscribe(
        (data) => {
  ...
  handleGetAllClients() {
    this.resourcesService.getAllResourcesPages(
    ...
    this.resourcesService
      ...
      .subscribe({
        next: (data) => {
          ...
          this.resources = data.content;
          this.totalPages = data.totalPages;
          localStorage.setItem('clients', JSON.stringify(this.resources));
  ...
  */
  handleSearchClient() {
    ...
    this.resourcesService.searchNote(keyword).subscribe({
      next: (data) => {
        this.resources = data.content;
        this.totalPages = data.totalPages;
      }
  ...
  handleSetPayed(c: any) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.resourcesService.editNote(c).subscribe({
          next : data => {
  ...
  handledeleteClient(r : any) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.resourcesService.deleteResource(r.id).subscribe({
          next : (data) => {
            //this.handleGetAllProducts()
            let index = this.resources.indexOf(r);
